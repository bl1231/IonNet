...
class SCOPER:
    ...
    def __init__(
        ...
        saxs_script_path: str,
        multifoxs_combination_script_path: str,
        add_hydrogens_script_path: str,
        multifoxs_script_path: str,
        kgs_k: int = 100,
        top_k: int = 1,
        multifoxs_run: bool = False,
        fixc1c2: bool = False,
    ):
        """
    ...
    def run(self):
        ...
        top_k_pdbs, kgs_db = KGSRNA(
            self.__kgsrna_work_dir,
            self.__pdb_path,
            self.__kgs_k,
            self.__saxs_script_path,
            self.__saxs_profile_path,
            self.__add_hydrogens_script_path,
            self.__top_k,
        ).compute()
        for pdb_file, _ in top_k_pdbs:  # already sorted
            fpath = os.path.join(
                os.getcwd(), os.path.join(os.getcwd(), os.path.join(kgs_db, pdb_file))
            )
...
class KGSRNA:
    ...
    def __init__(
        ...
        self.__kgsrna_script_path = "/home/scoper/IonNet/scripts/scoper_scripts/Software/Linux64/KGSrna/KGSrna --initial {}.HB --hbondMethod rnaview --hbondFile {}.HB.out -s {} -r 20 -c 0.4 --workingDirectory {}/"
        self.__pdb_path = pdb_path
    ...
    def preprocess(self):
        ...
        result = subprocess.run(
            f"{self.__addhydrogens_script_path} {self.__pdb_path}",
            shell=True,
        ...
        # print("KGS prepare")
        # result = subprocess.run(f"python /home/scoper/app/scripts/copy_of_kgs_prepare.py -v {self.__pdb_path}.HB", shell=True,
        #                         stdout=subprocess.PIPE,
        #                         stderr=subprocess.PIPE
        #                         )
        ...
        subprocess.run(
            f"{self.__rnaview_path} {self.__pdb_path}.HB",
            shell=True,
            env=my_env,
    ...
    def get_samples(self):
        ...
        print(f"Running KGS with {self.__kgs_k} samples, this may take a few minutes")
        print(
    ...
    def get_top_k(self, saxs_scores: dict):
        ...
        """
        sorted_scores = {
            k: v for k, v in sorted(saxs_scores.items(), key=lambda item: item[1])
        }
